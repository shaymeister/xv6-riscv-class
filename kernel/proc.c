...
// ORIGINAL PROC.C code without cfs-----------------------------------------------------------------------
#include "types.h"
#include "param.h"
#include "memlayout.h"
#include "riscv.h"
...
#include "defs.h"
//#include "file.c"
//include missing that are used in sysfile.c??
//#include "sysfile.c"
#include "fs.h"
#include "sleeplock.h"
#include "file.h"
...
int chprio(int pid, int priority)
{
    //calculateWeight throws an error
  //   struct proc *p;
  //   for(p = proc; p < &proc[NPROC]; p++){
...
//-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
//-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
//This code was copied from sysfile.c to be able to open files in proc.c

static int
p_argfd(int n, int *pfd, struct file **pf)
{
  int fd;
  struct file *f;
...
p_sys_unlink(void)
{
  ...
  if(ip->type == T_DIR && !p_isdirempty(ip)){
    iunlockput(ip);
    goto bad;
  }
...
static struct inode*
p_create(char *path, short type, short major, short minor)
{
  struct inode *ip, *dp;
  char name[DIRSIZ];
...
//copied sys_open method from sysfile.c
//returns the file descriptor of the opened file
//added #include sysfile.c so it 'should' still have access to the create method if a file doesn't exist
uint64 
proc_sys_open(void){
char path[MAXPATH] = "../procData.txt";
  //omode is open mode
  int fd, omode;
  struct file *f;
  struct inode *ip;
  ...
  if((f = filealloc()) == 0 || (fd = p_fdalloc(f)) < 0){
    if(f)
      fileclose(f);
    iunlockput(ip);
    end_op();
...
p_sys_mkdir(void)
{
  ...
  begin_op();
  if(argstr(0, path, MAXPATH) < 0 || (ip = p_create(path, T_DIR, 0, 0)) == 0){
    end_op();
    return -1;
  }
  iunlockput(ip);
...
//method to put together file sys_calls to open, write to, and close a file
//called from the scheduler and passes data from process on scheduler to be written to file
void writeProcFile(int data)
{
  // //FILE *fptr;
  //I think I will stil need this
  ...
  //need to use sys_open code here from sysfile.c to open a file
  //^^call copied code in proc_sys_open
  int fd = proc_sys_open();
  if (fd == -1)
